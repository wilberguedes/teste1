<x-layouts.installer>
    <div class="p-3">
        ...
        <div
            class="mt-4 mb-3 block w-full rounded-md border border-neutral-300 bg-neutral-50 py-4 px-5 text-base shadow-sm">
        ...
        <p class="mt-4 text-neutral-700">
            ...
            executable
            (for example, <code
                class="select-all bg-danger-100 px-2">/usr/local/bin/php{{ str_replace('.', '', config('installer.core.minPhpVersion')) }}</code>
            or <code
                class="select-all bg-danger-100 px-2">/opt/alt/php{{ str_replace('.', '', config('installer.core.minPhpVersion')) }}/usr/bin/php</code>instead
        ...
        @if (count($patches) > 0)
            ...
            <form class="mt-5 flex items-center" method="post" action="{{ url()->full() }}" id="patchForm">
                ...
                <button type="submit" id="applyPatchesBtn"
                    class="mt-6 ml-3 inline-flex items-center rounded-md border border-transparent bg-success-500 px-4 py-2 text-sm text-white shadow-sm focus:outline-none focus:ring-2 focus:ring-success-400 focus:ring-offset-2 disabled:pointer-events-none disabled:opacity-70 hover:bg-success-600">
                    Apply Patches
                </button>
