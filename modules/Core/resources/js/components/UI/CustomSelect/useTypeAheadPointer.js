...
export function useTypeAheadPointer(
  ...
  function maybeAdjustScroll() {
    const optionEl =
      dropdownMenuRef.value?.targetDomElement.children[typeAheadPointer.value] || false
    ...
    if (optionEl) {
      ...
      if (top < bounds.top) {
        return (dropdownMenuRef.value.targetDomElement.scrollTop = optionEl.offsetTop)
